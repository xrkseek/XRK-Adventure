...
var seed_scene: PackedScene = preload("res://entities/projectile/seed_bullet.tscn")


func _ready() -> void:
	anim.sprite_frames = SpriteFactory.make_player_frames()
	anim.play("idle")
	_sync_from_run()
...
func _physics_process(delta: float) -> void:
	# Invuln always ticks so upgrade pauses cannot freeze iframes.
	if _invuln > 0.0:
		_invuln -= delta
...
func _try_shoot() -> void:
	...
	for i in n:
		...
		var ang := dir.angle() + t * spread_ang
		var bullet := seed_scene.instantiate()
		bullet.global_position = origin
		bullet.setup(Vector2.from_angle(ang) * 520.0, damage, pierce)
		if scene and scene.has_method("spawn_projectile"):
			scene.spawn_projectile(bullet)
		else:
